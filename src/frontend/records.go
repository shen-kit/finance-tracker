...
// }
//
// func (rv recordsView) update() {
// 	rv.createHeaders()
// 	rv.maxPage = backend.GetRecordsPages() - 1
// 	recs := backend.GetRecordsRecent(rv.curPage)
//
...
// }
//
// func createRecordsTable() *recordsView {
// 	table := createMyTable(strings.Split(" ID : Date : Category : Description : Amount ", ":"))
// 	return &recordsView{
...
// }
//
// func (rv recordsView) setKeybinds(rf recordForm) {
// 	rv.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
// 		if isBackKey(event) {
// 			flex.RemoveItem(rv)
// 			app.SetFocus(flex)
// 		} else if event.Rune() == 'a' {
// 			showRecordForm(rv, rf, -1, "", "", "", "")
// 		} else if event.Rune() == 'd' { // delete record
// 			row, _ := rv.GetSelection()
// 			id := rv.getCellInt(row, 0)
// 			backend.DeleteRecord(id)
// 			rv.update()
// 		} else if event.Rune() == 'e' { // edit record
// 			row, _ := rv.GetSelection()
// 			id := rv.getCellInt(row, 0)
// 			date := rv.getCellString(row, 1)
// 			catName := rv.getCellString(row, 2)
// 			desc := rv.getCellString(row, 3)
// 			amt := rv.getCellString(row, 4)
// 			showRecordForm(rv, rf, id, date, desc, amt, catName)
